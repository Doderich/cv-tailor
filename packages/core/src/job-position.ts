...
export function inferJobPositionFromText(text: string): JobPosition {
	...
	if (
		/\b(mobile|ios|android|flutter|react[\s-]?native|swiftui|kotlin)\b/.test(
			normalized,
		)
	) {
		return "mobile";
	...
	if (
		/\b(devops|sre|site reliability|platform engineer|infrastructure)\b/.test(
			normalized,
		)
	) {
		return "devops";
	...
	if (
		/\b(machine learning|ml engineer|ai engineer|deep learning)\b/.test(
			normalized,
		)
	) {
		return "ml";
	...
	if (
		/\b(frontend|front-end|front end|react developer|vue developer|angular)\b/.test(
			normalized,
		)
	) {
		return "frontend";
	...
	if (
		/\b(backend|back-end|back end|node\.?js developer|java developer|golang)\b/.test(
			normalized,
		)
	) {
		return "backend";
	...
	if (
		/\b(product designer|ux designer|ui designer|product design)\b/.test(
			normalized,
		)
	) {
		return "design";
