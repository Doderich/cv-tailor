...
/// Optional progress callback used by desktop (Tauri events) or ignored by the gateway.
pub type ProgressSink = Arc<dyn Fn(AiRunProgressEvent) + Send + Sync>;
...
pub const AI_RUN_PROGRESS_EVENT: &str = "ai-run-progress";
...
fn schema_path(data_dir: &Path) -> Result<PathBuf, AppError> {
    let directory = data_dir.join("cv-tailor");
...
fn emit_run_progress(progress: Option<&ProgressSink>, run_id: &str, stream: &str, text: &str) {
    let Some(progress) = progress else {
        return;
    };

    progress(AiRunProgressEvent {
...
async fn collect_stream(
    ...
    while let Ok(Some(line)) = reader.next_line().await {
        if !line.trim().is_empty() {
            emit_run_progress(progress.as_ref(), &run_id, stream, &line);
        }
...
async fn collect_stderr(
    ...
    while let Ok(Some(line)) = reader.next_line().await {
        if !line.trim().is_empty() {
            emit_run_progress(progress.as_ref(), &run_id, "stderr", &line);
        }
        collected.push_str(&line);
        collected.push('\n');
    }
...
async fn run_with_stdin(
    ...
    finish_child(progress, run_id, tool, start, child, stdout, stderr).await
}
...
fn process_lm_studio_sse_line(
    ...
    if let Some(content) = extract_lm_studio_stream_content(&payload) {
        if !content.is_empty() {
            emit_run_progress(progress, run_id, "stdout", &content);
...
pub async fn run_ai_tool(
    ...
    let progress_app = if has_progress { progress.clone() } else { None };
    ...
    let schema_path = schema_path(data_dir)?;
